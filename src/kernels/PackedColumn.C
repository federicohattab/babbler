...
PackedColumn::validParams()
{
  ...
  // The inputs are parsed with the addRangeCheckedParam() method. This method is like addParam(), i.e.,
  // it sets a default value in lieu of user input (1, 7.98e-04 were used here). However, it has an
  // additional argument that accepts logical expressions, which operate on the parameter itself.
  // If the expression is false for a given input, an error message is invoked and the application terminates.
  // This method provides more convenient means for enforcing mu != 0 or 1 <= d <= 3 than hard cording a 
  // condition that leads to a paramError().
  params.addRangeCheckedParam<Real>(
      "diameter",
      1.0,
      "(1 <= diameter) & (diameter <= 3)",
      "The diameter (mm) of the steel spheres packed within the column that is used to compute "
      "its permeability. The input must be in the range [1, 3]. The defaut value is 1 mm.");
  params.addRangeCheckedParam<Real>(
      "viscosity",
      7.98e-04,
      "viscosity != 0",
